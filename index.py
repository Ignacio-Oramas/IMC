...
def recuperar_password():
    ...
    if form.validate_on_submit():
        username = form.username.data
        nueva_password = form.nueva_password.data
        
        db = get_db()
        user = db.execute('SELECT * FROM Entrenador WHERE username = ?', (username,)).fetchone()
        
        if user:
            db.execute('UPDATE Entrenador SET password = ? WHERE username = ?', (nueva_password, username))
            db.commit()
...
def eliminar_peso(peso_id):
    ...
    conn = get_db()
    conn.execute('DELETE FROM Pesos WHERE id = ?', (peso_id,))
    conn.commit()
    conn.close()
...
def grafica_usuario(dni):
    ...
    conn = get_db()
    query = 'SELECT mes, anio, peso FROM Pesos WHERE usuario_id = ?'
    params = [usuario['id']]
    
    if anio_inicio:
        query += ' AND CAST(anio AS INTEGER) >= ?'
        params.append(int(anio_inicio))
    if anio_fin:
        query += ' AND CAST(anio AS INTEGER) <= ?'
        params.append(int(anio_fin))
        
    query += ' ORDER BY anio, mes'
    
    pesos_raw = conn.execute(query, params).fetchall()
    conn.close()
    
    # Preparar datos para la gráfica
    pesos = {f"{registro['anio']}-{registro['mes']}": registro['peso'] for registro in pesos_raw}
    
    # Solo generar el grafo si existen datos en el rango
    if pesos:
        graphJSON = json_grafica(usuario['altura'], pesos)
    else:
        graphJSON = None
        flash('No hay datos en el rango seleccionado', 'error')
    
    return render_template('grafica.html', graphJSON=graphJSON, dni=usuario['dni'], anio_inicio=anio_inicio, anio_fin=anio_fin)
...
def crear_usuario_route():
    ...
    if form.validate_on_submit():
        # Validation for duplicate DNI
        usuario_existente = obtener_usuario_por_dni(form.dni.data)
        if usuario_existente:
            flash('Error: El DNI ya está registrado en el sistema.', 'error')
